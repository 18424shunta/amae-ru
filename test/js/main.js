...
var selected_person_id = 0;//プルダウンメニュー選択値
 ...
 *グラフデータ取得
 * ------------------------------------*/
parameter = { "s_date": selected_start_date, "e_date": selected_end_date, "person_id": selected_person_id };
POST_data(parameter, "api/get_catch_fish.php").then(function (data) {
  // console.log(data);
  fish_catch_chart(data['fish_name'], data['fish_sum']);
});
/* -------------------------------------
 *グラフの描画
 * ------------------------------------*/
function fish_catch_chart(label, data) {
  ctx = document.getElementById("fish_catch_count");

  fish_catch_count_chart = new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: label, // fish_name_list,
      datasets: [{
        backgroundColor: [
          "#BB5179",
          "#FAFF67",
...
function updateChart(chart) {
  ...
  POST_data(parameter, "api/get_catch_fish.php").then(function (data) {
    // console.log(data);
    if (!fish_catch_count_chart) {
      fish_catch_chart(data['fish_name'], data['fish_sum']);
    }
    else {
      fish_catch_count_chart.destroy();
      fish_catch_chart(data['fish_name'], data['fish_sum']);
    }
  });
}
...
function plot_marker() {
  var parameter = { "s_date": selected_start_date, "e_date": selected_end_date, "person_id": selected_person_id };
  POST_data(parameter, "api/get_gps_chart.php").then(function (data) {
